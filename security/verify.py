...
def load_enrolled_embedding(path=DEFAULT_ENROLLED_PATH):
...
def run_verification(
    enrolled_path=DEFAULT_ENROLLED_PATH,
    threshold=0.6
):
    ...
    try:
        ...
        print(f"Enrollment file not found: {enrolled_path}")
        print("Please run enrollment first.")
        return
    ...
    while True:
        ...
        if last_payload:
            ...
            cv2.putText(
                frame,
                f"Identity: {last_payload['verdict']}",
                (20, 40),
            ...
            cv2.putText(
                frame,
                f"Score: {last_payload['identity_score']}/100",
                (20, 75),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (200, 200, 200),
            ...
            cv2.putText(
                frame,
                f"Confidence: {last_payload['confidence']}",
                (20, 110),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (200, 200, 200),
